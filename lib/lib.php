...
class mangashowMeGet{
  ...
  {
    $temp = substr($this->Dom,strpos($this->Dom,"https:\/\/"),strpos($this->Dom,"view_cnt")-strpos($this->Dom,"https:\/\/")-9);
    $temp = str_replace("\\","",$temp);
    do {
      $result[] = substr($temp,0,strpos($temp,"\",\""));
      $temp = substr($temp,strpos($temp,"\",\"")+3);
  ...
  {
    preg_match("/\<title\>(.*)\<\/title\>/",$this->Dom,$result);
    return $result[1];
  }
}
class DirectZip{
